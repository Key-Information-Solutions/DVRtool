...
**Written:** 2026-08-25, from a related internal (operator) session.
**For:** a DVRTool session implementing the write-side automation so Site A's onboarding
...
`hikvision-access-control-findings.md` §5a (canary on the third panel).
...
- **The policy**, freshly pulled and validated against the live panels:
  ...
  ```json
  { "group": "Group B", "groupGuid": "...", "scheduleGuid": "...",
...
| **ocb2** | 192.0.2.**223** | `BBBB2222…` | V2.0.4 | HR hallway / Interior hallway / Department office 2 / Department office 3 |
| **ocb3** | 192.0.2.**222** | `CCCC3333…` | V2.0.9 | Manager office 2 / IT room / Conference room / Reception |
...
| Group B | 143 | ocb1/.221 door 1 only | 24/7 |
| Group C | 4 | ocb1/.221 door 4 | ~24/7 (00:02–24:00) |
| Group D | 7 | ocb3/.222 door 2 | 24/7 |
| Group E | 5 | ocb3/.222 door 1 | 24/7 |
...
enumeration (findings §5): Super admin 6×3 = the 18 all-four rows, Group B = the 141 door-1
rows, and the Group C/D/E overlaps produce the 1+2 / door-2 / 1+4 rows. The third panel = the 6
Super admins only. So the model is correct and current.
...
1. **Do not fire writes at the live panels in this implementation task.** Build the code, run the
   **unit tests**, and validate only with `--dry-run` and the read-only `reconcile`. The first real
   write (a throwaway test fob on the quiet third panel **/ ocb2**, created then revoked, byte-diff
...
- `access onboard … --dry-run` prints, for a sample person+groups, the correct per-panel door
  union (e.g. a Group D+E person → .222 doors 1 and 2; a Group B person → .221 door 1).
...
- Schedule handling is the documented simplification: grants are plan-1 (24/7); a non-24/7 group is
  written **always-on with a warning** (the live Group C, `00:02:00` start, triggers it).

`--dry-run` is the default for onboard/offboard; a write needs `--force` **and** no `--dry-run`.
Verified dry-run outputs (no panel contact): Group B → `.221` door 1; Group D + Group E → `.222`
doors 1,2; Super admin → all 4 doors on `.221/.222/.223`; unknown group → clear error.

## 10. First live write — the third-panel / ocb2 canary (run with operator go-ahead only)

The low-level write path (`UpsertCardAsync` / `RevokeCardAsync`) is already proven on the third
panel — see `hikvision-access-control-findings.md` §5a. This is the procedure for the
**provisioning-verb** canary, on the quietest panel (6 fobs), from **the relay host** with the
published exe:
...
2. **Snapshot the third panel.** `dvrtool access export --panel 192.0.2.223 --out before.csv` (customer data —
...
3. **Dry-run the canary.** Use a throwaway fob that is **not** in use (findings used `9001`) and the
   lowest-level verb so the canary touches the third panel only — `access grant --card 9001 --doors 1
   --panel 192.0.2.223 --name Test.Canary` (no `--force`) — and read the planned write.
   (`onboard` targets whichever panels a group maps to; only *Super admin* reaches the third panel,
   which is too broad for a canary. For an onboard-verb canary instead, hand-write a one-group policy
   whose only door is `192.0.2.223` door 1 and `onboard --policy that.json --card 9001 --group <it>
   --dry-run`.)
...
The third-panel / ocb2 canary in §10 was executed on **the relay host** (the panel-LAN machine, via
the remote-agent relay) with the shipped `C:\Program Files\DVRTool\cli\dvrtool.exe`, operator go-ahead
given. **This is the first live write to a physical panel, and it rolled back byte-clean.** Sequence
and results:

1. `access reconcile` — reached and serial-pinned all three panels; **third panel in sync (6/6)**.
   First panel (144 vs 149) and second panel (14 vs 15) showed **explained** drift only: 5 policy
   members have no unique fob in the identity map (fobs incl. the known straggler), so their live
   fobs read as "extra". Not a resolver error — an identity-map completeness gap. Resolver matches
   reality.
2. Snapshot third panel → `before.csv` (6 fobs). Confirmed fob **9001 absent fleet-wide** before touching it.
3. Dry-run `grant --card 9001 --doors 1 --panel 192.0.2.223` → planned third-panel-only add; refused
...
5. Independent `find --card 9001` → present on **the third panel only**, active.
6. `revoke --card 9001 --force` → verified **gone**; `find` confirms absent fleet-wide.
7. Re-export third panel → `after.csv`; **SHA-256 of before == after** (byte-identical). Rollback
   proof, as findings §5a saw with the low-level path.
...
- **`AccessReconciler.BuildExpectation` groups members by name and set-matches interchangeable fobs.**
  ...
  several distinct people share a name *and need the identical door set* (two people in Group B, say),
  their fobs are interchangeable, so each is expected with that shared union — the fob **set** is
  verified even though the fob↔person link is not. Only a shared name whose holders need *different*
  doors stays unmapped (`GrantKey` compares the per-panel door signatures).

Verified on the live fleet from a side-built CLI (installed app untouched, scratch deleted):
first panel **149/149**, second panel **15/15**, third panel **6/6** — **RESULT: in sync**, exit 0.
The only remaining line is the intentional non-24/7 note for Group C (the documented schedule
...
then re-verified against the live fleet: first panel 149/149, second panel 15/15, third panel 6/6 —
**in sync**. Note the appended `…\DVRTool\cli` PATH entry reaches only shells started after the
install (the long-running remote-agent process keeps its old env until relaunched); the discovery
registry key and full path work regardless. Decide the iVMS sync direction (§10 safety note) before
routine provisioning if the GUI is still used.
